...
void
_kite_iconify(GLFWwindow *window, int iconified) {
	lua_State *L = kite->L;
	if (iconified)
...
void
kite_textinput(GLFWwindow *window, uint32_t code) {
	lua_State *L = kite->L;
...
load_window_conf(lua_State *L) {
	...
	if (icon) {
		kite->conf.window.icon = malloc(strlen(icon) + 1);
		strcpy(kite->conf.window.icon, icon);
	} else {
		kite->conf.window.icon = NULL;
	}
}
...
create_kite(const char *gamedir) {
	...
	kite->_iconify = _kite_iconify;
	kite->mouse = kite_mouse;
	kite->keyboard = kite_keyboard;
	kite->textinput = kite_textinput;
